...
var QueryViewerMapType = { Choropleth: "Choropleth", Bubble: "Bubble", Point: "Point" };
...
qv.util = {
	...
	LoadColorsObj: function (colorsObj, styleSheet, rulePrefix) {
		var colorsCount = 0;
	...
	GetHighchartsDefaultColors: function (qViewer) {
		if (!qViewer._HighchartsDefaultColors) {
			...
			qv.util.LoadColorsObj(colorsObj, styleSheet, rulePrefix);
			if (qViewer.Class != "") {
	...
	SetHighchartsColor: function (qViewer, highchartsObject, color, colorIndexForDefaultColors) {
		if (!qv.util.IsNullColor(color)) {
			var colorIndex;
			if (color.IsDefault)
				colorIndex = color.ColorIndex;
			else
				colorIndex = qv.util.AddHighchartsCustomColor(qViewer, color.Color);
	...
	AddHighchartsCustomColor: function (qViewer, color) {
		...
		if (localColorIndex < 0) {
			...
			var prefix = qv.util.IsHexaColor(color) ? "#" : "";
	...
	GetColorStringFromHighchartsObject: function(qViewer, highchartsObject) {
		...
		if (colorIndex < HIGHCHARTS_MAX_COLORS) {
			var defaultColors = qv.util.GetHighchartsDefaultColors(qViewer);
			color = defaultColors[colorIndex];
		}
		else {
			color = HIGHCHARTS_CUSTOM_COLOR[colorIndex - HIGHCHARTS_MAX_COLORS];
			if (qv.util.IsHexaColor(color))
				color = "#" + color;
		}
		return color;
	},
	...
	getTitleObject: function (qViewer, serieIndex) {
		var title;
		if (qViewer.Title == "")
			title = { text: null };
		else {
	...
	ProcessDataAndMetadata: function (qViewer) {
		...
		function GetDataByDataFieldObj(qViewer, uniqueAxis) {
			...
			function IsMulticoloredSerie(qViewer, datum, uniqueAxis) {
				...
				if (qViewer.RealType == QueryViewerOutputType.Map && ((qViewer.MapType == QueryViewerMapType.Choropleth && qViewer.Chart.colorAxis != "") || (qViewer.MapType == QueryViewerMapType.Bubble && qViewer.Chart.colorAxis != "") || (qViewer.MapType == QueryViewerMapType.Point && qViewer.Chart.colorAxis != "")))
				...
				else if (qViewer.RealType == QueryViewerOutputType.Map && ((qViewer.MapType == QueryViewerMapType.Choropleth && (qViewer.Chart.colorAxis.dataClasses.length == 0)) || (qViewer.MapType == QueryViewerMapType.Bubble && (qViewer.Chart.colorAxis.dataClasses.length == 0)) || (qViewer.MapType == QueryViewerMapType.Point && (qViewer.Chart.colorAxis.dataClasses.length == 0))))
		...
		function XAxisDataTypeOK(qViewer) {
			...
			switch (qViewer.RealType) {
				case QueryViewerOutputType.Chart:
					...
					break;
				case QueryViewerOutputType.Map:
					dataTypeOK = dataType == QueryViewerDataType.Character;
					errCode = "GXPL_QViewerNoCharacterAxis";
					break;
				default:
					dataTypeOK = true;
					break;
...
qv.services = (function () {
	...
	function ServicesVersionOK(version) {
		return version == null || parseInt(version) == METADATA_VERSION;		// Se controla que sea distinto de null por la ejecuci�n de consultas a trav�s del API de GXquery, que no devuelven versi�n por ser servicios m�s viejos
	}
...
qv.chart = (function () {
	...
	function CircularGaugeTooltipAndDataLabelFormatter(evArg, qViewer) {
		...
		return qv.util.dom.createSpan(null, "", "", "", { color: qv.util.GetColorStringFromHighchartsObject(qViewer, evArg), fontSize: fontSize + "px" }, null, qv.util.formatNumber(evArg.point.y, 2, "ZZZZZZZZZZZZZZ9.99", true) + "%").outerHTML;
	}
	...
	function TooltipFormatter(evArg, chartSeries, sharedTooltip)
	{
		...
		if (!sharedTooltip) {
			...
			var res = "";
			if (hasTitle)
				res += GetBoldText(x, isSingleSerieChart ? qv.util.GetColorStringFromHighchartsObject(qViewer, evArg) : "");
			for (var i = 0; i < hoverPoints.length; i++) {
				var point = hoverPoints[i];
				var serie = chartSeries[point.series.name];
				res += (hasTitle || i > 0 ? '<br/>' : '') + GetBoldText(point.series.name + ': ', isSingleSerieChart ? "" : qv.util.GetColorStringFromHighchartsObject(qViewer, point));
	...
	function PieTooltipFormatter(evArg, chartSeries, sharedTooltip) {
		if (!sharedTooltip) {
			...
			for (var i = 0; i < hoverPoints.length; i++) {
				var point = hoverPoints[i];
				var serie = chartSeries[point.series.name];
				res += (hasTitle || i > 0 ? '<br/>' : '') + GetBoldText(point.series.name + ': ', isSingleSerieChart ? "" : qv.util.GetColorStringFromHighchartsObject(qViewer, point.series));
	...
	function DateTimeTooltipFormatter(evArg, chartSeries) {
		...
		for (var key_name in points_by_name) {
			...
			var color = qv.util.GetColorStringFromHighchartsObject(qViewer, points[0]);
			if (compare)
	...
	function getHighchartOptions(qViewer, chartSerie, serieIndex) {
				...
				if (qViewer.RealChartType == QueryViewerChartType.LinearGauge) {
					...
					for (var serieName in qViewer.Chart.Series.ByName) {
						if (!IsSplittedChart(qViewer) || i == serieIndex) {
							...
							qv.util.SetHighchartsColor(qViewer, plotBand, color, false)
					...
					for (var i = 0; i < qViewer.Chart.PlotBands.length; i++) {
						...
						if (chartSerie == null || chartSerie.Name == chartPlotBand.SeriesName) {
							if (chartPlotBand.From == chartPlotBand.To && chartPlotBand.From != null) {
								...
								qv.util.SetHighchartsColor(qViewer, plotLine, chartPlotBand.Color, false);
								yAxis.plotLines.push(plotLine);
							}
							else {
								var plotBand = {};
								qv.util.SetHighchartsColor(qViewer, plotBand, chartPlotBand.Color, false);
		...
		function getSeriesObject(qViewer, serieIndex, groupOption) {
			...
			function getSerieObject(qViewer, chartSerie, serieIndex, series, groupOption) {
				...
				if (IsTimelineChart(qViewer)) {
					...
					if (!qv.util.IsNullColor(chartSerie.Color))
						qv.util.SetHighchartsColor(qViewer, serie, chartSerie.Color, true);
					...
					for (j = 0; j < points.length; j++) {
						...
						if (qv.util.IsNullColor(chartSerie.Color))
							qv.util.SetHighchartsColor(qViewer, serie.data[j], chartSerie.Points[j].Color, true);
					}
				}
				else {
					var widths;
					...
					for (j = 0; j < chartSerie.Points.length; j++) {
						...
						if (qViewer.RealChartType == QueryViewerChartType.CircularGauge) {
							...
							if (!qv.util.IsNullColor(chartSerie.Color))
								...
								color = chartSerie.Points[0].Color;
							qv.util.SetHighchartsColor(qViewer, serie.data[j], color, true);
						}
				...
				for (var serieName in qViewer.Chart.Series.ByName) {
					if (!IsSplittedChart(qViewer) || i == serieIndex) {
						...
						if (!qv.util.IsNullColor(chartSerie.Color))
							...
							color = chartSerie.Points[0].Color;
						qv.util.SetHighchartsColor(qViewer, oneBackground, color, false);
		...
		options.title = qv.util.getTitleObject(qViewer, serieIndex);
...
qv.map = (function () {
	...
	//Los PointMap tienen un array de datos distintos a los otros mapas ya que tiene 4 datos en lugar de 2
	function dataPointMap(qViewer) {

		var array = qViewer.Data.Rows;
		var dataPoint = [];
		var objAux;
		for (var i = 0; i < array.length; i++) {
			...
			objAux = {
				...
				lat: array[i].F2,
				lon: array[i].F3,
				colorValue: array[i].F4
			};
			dataPoint.push(objAux);
	...
	function choroplethMapSerie(qViewer) {
		...
		var serie;
			...
			serie = [
				{
					name: qv.util.getTitleObject(qViewer, 0).text,
					type: 'map',
					borderColor: 'gray',
					colorByPoint: false,
	...
	//Devuelve la serie de un PointMap
	function pointMapSerie(qViewer) {

		var series;
		//En el caso de ser PointData su serie va a ser distinta a si fuese un PointColor
		if (qViewer.Chart.colorAxis != "") { //is a point color map
			...
			series = [{
				...
				name: getTitleObject(qViewer, 0).text,
				type: 'mappoint',
				colorKey: 'colorValue',
				data: dataPointMap(qViewer)
				}];

		} else { //is a point data map
	...
	function getColorAxis(qViewer) {
		...
		if ((qViewer.MapType == QueryViewerMapType.Choropleth && qViewer.Chart.colorAxis.dataClasses.length != 0) || (qViewer.MapType == QueryViewerMapType.Bubble && qViewer.Chart.colorAxis.dataClasses.length != 0) || (qViewer.MapType == QueryViewerMapType.Point && qViewer.Chart.colorAxis.dataClasses.length != 0)) { // is a color scale map, bubble color map or point color map

			colorAxis = qViewer.Chart.colorAxis;
			return colorAxis;
		}

		if ((qViewer.MapType == QueryViewerMapType.Bubble && qViewer.Chart.colorAxis.dataClasses.length == 0) || (qViewer.MapType == QueryViewerMapType.Point && qViewer.Chart.colorAxis.dataClasses.length == 0)) { // is a color scale map, bubble color map or point color map
	...
	function getReferencyMap(qViewer) {
		...
		if (qViewer.MapType == QueryViewerMapType.Point) {

			var legend;

			if (qViewer.Chart.colorAxis.dataClasses.length != 0) { // is a point color map
	...
	function getToolTipMaps(qViewer) {
		...
		if (qViewer.MapType == QueryViewerMapType.Point) {
			...
			tooltip = {
				headerFormat: '',
				pointFormat: '<b>{point.name}</b><br>Lat: {point.lat}, Lon: {point.lon}, value:{point.colorValue}'
			}
			return tooltip;
		}
	...
	function getChartConfig(qViewer) {
		...
		if ((qViewer.MapType == QueryViewerMapType.Bubble && qViewer.Chart.Series.ByIndex[0].Color.Color) || (qViewer.MapType == QueryViewerMapType.Choropleth && qViewer.Chart.colorAxis.dataClasses.length != 0 && !qViewer.Chart.Series.ByIndex[0].Color.Color) || (qViewer.MapType == QueryViewerMapType.Bubble && qViewer.Chart.colorAxis.dataClasses.length != 0 && !qViewer.Chart.Series.ByIndex[0].Color.Color)) {
			chart = {
				map: getMapDirectory(qViewer).mapKey,
				styledMode: false
			}
		} else {
			chart = {
	...
	function mapaHighchartsOptions(container, qViewer) {
		...
		Highcharts.mapChart(container, {
			...
			title: qv.util.getTitleObject(qViewer, 0),
	...
	//funcion que devuelve el directorio del mapa seleccionado para su posterior carga dinamica
	function getMapDirectory(qViewer) {

		var dir = (qViewer._EmbeddedInDashboard ? "DashboardViewer/" : "") + "QueryViewer/" + (qViewer.MapLibrary === QueryViewerMapLibrary.Highchart ? "highcharts/" : "Echarts/");
		var key;
		var mapa = {};
	...
	function getDataRange(qViewer, textStyle) {
		...
		if ((qViewer.MapType == QueryViewerMapType.Choropleth && qViewer.Chart.colorAxis.dataClasses.length != 0) ) { //color Scale
			...
			dataRange = {
				min: 0,
				max: qViewer.Chart.Series.ByIndex[0].MaxValue,
				calculable: true,
				orient: 'horizontal',
				left: 'center',
	...
	function getSerieMapEchart(qViewer, selectionColor, hoverColor) {

		var serieColor = getColorSerie(qViewer);
		...
		var serie = {
			...
			itemStyle: {
				...
				emphasis: { label: { show: false }, areaColor: qViewer.MapType === QueryViewerMapType.Choropleth ? hoverColor : '#eee' }
			},
			select: {
				label: {
					show: false
		...
		if (qViewer.MapType === QueryViewerMapType.Bubble) {
			...
			serie.markPoint = {
				...
				itemStyle: {
					normal: {
						borderColor: qv.util.color.lightenDarkenColor(serieColor, 50),
						borderWidth: 1,
						label: {
							show: false
						}
					...
					emphasis: {
						borderColor: qv.util.color.lightenDarkenColor(serieColor, 100),
						borderWidth: 2,
						label: {
							show: false
						}
	...
	function convertData (data,chart,qViewer) {
		...
		for (var j = 0; j < data.length; j++) {
			...
			for (var i = 0; i < chart._coordSysMgr._coordinateSystems[0].regions.length; i++) {
				if (chart._coordSysMgr._coordinateSystems[0].regions[i].name == name) {
					if (qViewer.Chart.colorAxis.dataClasses.length != 0) {
						for (var k = 0; k < qViewer.Chart.colorAxis.dataClasses.length; k++) {
							var from = qViewer.Chart.colorAxis.dataClasses[k].from;
							var to = qViewer.Chart.colorAxis.dataClasses[k].to;
							...
							if (from < value && value < to) {
								if (chart._coordSysMgr._coordinateSystems[0].regions[i].properties.center != undefined) {
									res.push({
										...
										value: data[j].value,
										itemStyle: {
											color: qViewer.Chart.colorAxis.dataClasses[k].color
										}
									});
								} else {
									res.push({
	...
	function mapaEchartsOptions(container, qViewer) {
		...
		function getThemeDefinitions(qViewer) {
			...
			for (var i = 0; i < rules.length; i++) {
				...
				if (rules[i].selectorText === "." + containerControlClass) {
					...
					if (cssStyle.fontSize)
						textStyle.fontSize = cssStyle.fontSize;
			...
			return { TextStyle: textStyle, SelectionColor: selectionColor, HoverColor: hoverColor, BackgroundColor: backgroundColor };
		}

		function getTitleObject(qViewer, textStyle) {
			return {
				text: gx.getMessage(qViewer.Title),
				left: "center",
				textStyle: textStyle
			};
		}

		function getTooltipObject(textStyle) {
			return {
				trigger: 'item',
				textStyle: textStyle,
					...
					for (var i = 0; i < chart._coordSysMgr._coordinateSystems[0].regions.length; i++) {
						if (chart._coordSysMgr._coordinateSystems[0].regions[i].name == params.name) {
							if (Number.isNaN(params.value)) {
								...
								return chart._coordSysMgr._coordinateSystems[0].regions[i].properties.name + ' : ' + params.value;
							}
						}
					}
				}
		...
		var option = {
			...
			series: getSerieMapEchart(qViewer, themeDefinitions.SelectionColor, themeDefinitions.HoverColor),
			title: getTitleObject(qViewer, themeDefinitions.TextStyle),
			dataRange: getDataRange(qViewer, themeDefinitions.TextStyle),
			tooltip: getTooltipObject(themeDefinitions.TextStyle)
		};
							...
							if (qViewer.Metadata.ParserResult.Success) {
								qv.services.GetDataIfNeeded(qViewer, function (resText, qViewer) {		// Servicio GetData
									...
									if (!qv.util.anyError(resText)) {
										if (newData)
											qv.services.parseDataXML(qViewer);
											gx.http.loadScript(getMapDirectory(qViewer).mapDir, renderMap);
									} else {
										// Error en el servicio GetData
										errMsg = qv.util.getErrorFromText(resText);
										qv.util.renderError(qViewer, errMsg);
